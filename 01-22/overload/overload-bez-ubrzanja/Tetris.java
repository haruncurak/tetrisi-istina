	...
	private void restart() 
{
    ...
    // Reset these values to their defaults for a new game
	...
	public void clearRows() 
{
    ...
    for (int j = 22; j > 0; j--) {
        ...
        if (!gap) {
            deleteRow(j); // Clear the row
            j += 1; // Recheck the current row
            numClears += 1;
        }
    }

    // Award points based on the number of rows cleared simultaneously
    switch (numClears) {
        case 1:
            score += 100;
            break;
    ...
    // Note: No calls to adjustDifficultyForFlow() or changes to sleepTime
}
...
private void adjustDifficultyForFlow() {
    // Do nothing to prevent acceleration
}
